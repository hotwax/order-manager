...
## 2026-08-08 - Adding loading states to destructive actions\n**Learning:** Destructive actions, such as deleting a customer profile, often use an icon-only button (e.g., trash icon) without visual feedback during asynchronous processing. In Vue and Ionic applications, these async processes typically have a reactive state flag (e.g., `deleting`).\n**Action:** When inspecting destructive or asynchronous actions in icon-only buttons, replace the default icon with an `<ion-spinner>` when the async state is true. Ensure the `<ion-spinner>` has the `slot="icon-only"` attribute to maintain proper alignment during loading.## 2024-05-24 - External Link Pattern Refactoring
**Learning:** Raw HTML `<a>` tags should not be used for external links in this Ionic application. When raw anchors are used, they can sometimes cause minor styling or focus inconsistencies compared to native Ionic components.
**Action:** When finding raw HTML `<a>` tags used as icon buttons, refactor them into `<ion-button fill="clear">` elements. Ionic supports passing `href`, `target`, and `rel` to `<ion-button>` and will internally render a compliant, accessible anchor tag while maintaining the application's consistent styling and keyboard navigation.
