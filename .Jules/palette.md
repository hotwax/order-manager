...
## 2024-09-12 - Internal vs External Link Indicators
**Learning:** Found instances where the `openOutline` icon was mistakenly used on internal `router-link` buttons (e.g., "View order", "View customer"). Using `openOutline` on internal navigation breaks app-wide convention and confuses the established affordance, which teaches users that this specific icon means "navigates away" to an external tab/resource.
**Action:** Only use `<ion-icon slot="end" :icon="openOutline" />` on links that point to external resources (typically `href` with `target="_blank"`). Never use it on internal `router-link` navigation to ensure the affordance remains consistent and predictable.
