...
## 2026-08-08 - Adding loading states to destructive actions\n**Learning:** Destructive actions, such as deleting a customer profile, often use an icon-only button (e.g., trash icon) without visual feedback during asynchronous processing. In Vue and Ionic applications, these async processes typically have a reactive state flag (e.g., `deleting`).\n**Action:** When inspecting destructive or asynchronous actions in icon-only buttons, replace the default icon with an `<ion-spinner>` when the async state is true. Ensure the `<ion-spinner>` has the `slot="icon-only"` attribute to maintain proper alignment during loading.## 2024-06-25 - Ionic Spinner Layouts
**Learning:** When adding `<ion-spinner>` to an icon-only `<ion-button>`, you must include `slot="icon-only"` on the `<ion-spinner>` as well. If it is omitted, Ionic's layout engine handles the component incorrectly during the loading state, leading to awkward spacing or visual jumps.
**Action:** Always ensure `<ion-spinner>` elements have `slot="icon-only"` when acting as a loading state for an icon-only button.
