...
## 2024-09-10 - Adding Global Loading Feedback for Modal-Triggered Async Actions
**Learning:** Found several async actions that rely on user confirmation via an `<ion-modal>` or `alertController` where the application sits with no visual feedback while a network request processes after the modal closes. Using the application's global loader (`emitter.emit('presentLoader', { message: '...' })`) is standard here to avoid user confusion.
**Action:** Always wrap async API calls executed after a user confirms a modal dialog with `emitter.emit('presentLoader')` and `emitter.emit('dismissLoader')` in a `finally` block to provide system status feedback.
