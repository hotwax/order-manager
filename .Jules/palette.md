...
## 2024-05-24 - External Link Affordances and Security
**Learning:** Buttons linking to external resources using `href` but missing `target="_blank"`, `rel="noopener noreferrer"`, and an external link icon (`openOutline`) can lead to security vulnerabilities (reverse tabnabbing) and a poor user experience (lack of visual indication).
**Action:** When working on buttons with external `href`s, always ensure they include `target="_blank"`, `rel="noopener noreferrer"`, and `<ion-icon slot="end" :icon="openOutline" />`. Ensure you do this in one file per PR to avoid conflicts.
