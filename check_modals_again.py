...
for root, _, files in os.walk('src'):
    for file in files:
        if file.endswith('.vue'):
            ...
            fab_buttons = re.finditer(r'<ion-fab-button([^>]*)>(.*?)</ion-fab-button>', content, re.DOTALL)
            for match in fab_buttons:
                attrs = match.group(1)
                inner = match.group(2)
                if 'disabled' not in attrs and ('save' in attrs.lower() or 'submit' in attrs.lower() or 'confirm' in attrs.lower() or 'save' in inner.lower() or 'submit' in inner.lower() or 'confirm' in inner.lower()):
                    if '@click' in attrs:
                        print(f"Potential unguarded save FAB button in {path}: <ion-fab-button{attrs}>{inner.strip()}</ion-fab-button>")
