...
| `54191:288203` | Swap queue page | `src/views/SwapOrders.vue`, `src/components/tasks/SwapTaskCard.vue`, and `src/components/swaps/SuggestedProductActionPopover.vue`. | Aligned in PRs #92, #94, #95, #96, #97, #100, #101, #104, and #105; progress renders from task data when provided, headings fall back to order identifiers when order names are missing, Park passes the task id to the parking endpoint, Swap status/footer colors match the nested card, and empty non-date filter selects render the Figma `Select` resting text. |
...
| `54191:295583` | Hold queue page | `src/views/HoldOrders.vue` and `src/components/tasks/HoldTaskCard.vue`. | Mostly aligned; resolution comments are wired in PR #88, filter date selectors in PR #97, task heading/date fallback in PR #100, assigned-date display in the assignee row is present, the task id chip copy affordance is in PR #102, and empty non-date filter selects render `Select` in PR #105. Assign action is blocked by backend contract. |
| `54190:216632` | Bad Address queue page | `src/views/BadAddressOrders.vue` and `src/components/tasks/BadAddressTaskCard.vue`. | Aligned in PRs #84, #97, #100, #101, and #105: two Ionic address lists, radio choice, editable rows, filter date selectors, empty non-date filter select resting text, footer actions, task heading fallback, and Park task id handoff. |
| `54642:47115` | Fraud queue page | `src/views/FraudOrders.vue` and `src/components/tasks/FraudTaskCard.vue`. | Aligned in PRs #81, #100, #102, #103, and #105: three content columns, suggested-action footer item, readable order heading/date values when the API omits `orderName`, shared task id chip copy affordance, Figma-mapped payment/risk rows, and empty filter selects rendering `Select`. |
...
- Served the current #91 checkout, based on #105, from a detached AccxUI validation worktree at `http://127.0.0.1:8120`.
