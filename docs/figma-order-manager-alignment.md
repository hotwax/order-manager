...
| `54223:47293` | `54191:288203` | Shared Order Manager menu groups `Funnel`, `Blocked` queues, `In progress` queues, `Find` records (`Orders`, `Customers`), and `Settings`. | `Menu.vue` maps the Blocked queue group, In progress queue group, and PR #121 moves Orders/Customers into a `Find` divider after In progress while keeping Create order available under the same group so existing functionality is preserved. |
...
- A follow-up shared menu review found `54223:47293` groups record lookup links under a `Find` divider after the `In progress` queue group. PR #121 moves Orders and Customers into that group and keeps Create order there without adding missing Returns or Shipments routes.
