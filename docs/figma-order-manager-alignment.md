...
| `54223:47293` | `54191:288203` | Shared Order Manager menu groups `Funnel`, `Blocked` queues, `In progress` queues, `Find` records (`Orders`, `Customers`), and `Settings`. Blocked and In progress queue rows use end-slot count text; Unfillable includes the `swappable` qualifier. | `Menu.vue` maps the Blocked queue group, In progress queue group, and PR #121 moves Orders/Customers into a `Find` divider after In progress while keeping Create order available under the same group so existing functionality is preserved. PR #121 also adds `ion-note slot="end"` counts backed by existing dashboard/workflow stores. |
