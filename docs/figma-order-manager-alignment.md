...
| `54187:184360` | `Order / Holds` order-detail frame | `src/views/OrderDetail.vue` maps the header identity item, summary cards, timeline, `ion-segment`, and Holds segment task-card rendering. | Broad structure aligned; PR #106 aligns the ship-group segment label to the Figma `Shipgroups` copy; task-card details are tracked by queue-specific rows below. |
