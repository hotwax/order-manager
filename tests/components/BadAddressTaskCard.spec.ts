...
describe('bad address task card', () => {
  ...
  it('shows the ship group facility and carrier shipping method before the addresses', () => {
    ...
    expect(cardSource).toContain("carrierName(carriers.value, task.carrierPartyId)");
    expect(cardSource).toContain("shipmentMethodDescription(shipmentMethodTypes.value, methodId)");
  });
});
