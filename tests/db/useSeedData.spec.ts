...
async function seedDb(): Promise<void> {
  ...
  await c.bulkPut('facilityTypes', [{ facilityTypeId: 'WAREHOUSE', parentTypeId: 'PHYSICAL', syncedAt: 1 }]);
  ...
  await c.bulkPut('carriers', [
    { partyId: 'UPS', groupName: 'UPS', syncedAt: 1 },
    { partyId: 'P1', firstName: 'Ada', lastName: 'Lovelace', syncedAt: 1 },
  ]);
  await c.bulkPut('shipmentMethodTypes', [{ shipmentMethodTypeId: 'GROUND', description: 'Ground', syncedAt: 1 }]);
  await c.bulkPut('carrierShipmentMethods', [
  ...
  await c.bulkPut('statusFlowTransitions', [
    { transitionKey: 'a', statusId: 'ORDER_CREATED', toStatusId: 'ORDER_APPROVED', transitionSequence: 2, syncedAt: 1 },
    { transitionKey: 'b', statusId: 'ORDER_CREATED', toStatusId: 'RETURN_ACCEPTED', transitionSequence: 1, syncedAt: 1 },
  ]);
  await c.bulkPut('productStores', [{ productStoreId: 'STORE', storeName: 'Demo store', syncedAt: 1 }]);
  await c.bulkPut('productStoreFacilities', [
